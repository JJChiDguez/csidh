...
/* ------------------------------------------------------------- *
   ...
   output:
            ...
            0 if YP != ZP
 * ------------------------------------------------------------- */
int isZero_Point(const proj P)
{
	return (0 == compare((uint64_t *)P[0], (uint64_t *)P[1], NUMBER_OF_WORDS));
};
...
void elligator(proj T_plus, proj T_minus, const proj A)
{
	...
	// ---
	fp tmp, Cu2_minus_1, tmp_0, tmp_1;
