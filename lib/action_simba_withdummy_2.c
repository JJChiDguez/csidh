...
void action_evaluation(proj C, const uint8_t key[], const proj A)
{
	...
	while (isog_counter < NUMBER_OF_ISOGENIES)
	{
		...
		if( areEqual(current_A[0], E) == 1)
		{
			point_copy(current_T[0], T_m);
			point_copy(current_T[1], T_p);
		}
		else
			...
			else
			{
				...
				for (j = (i + 1); j < size_of_each_batch[m]; j++)
				{
					if( finished[batches[m][j]] == 0 )
					{
						...
						yMUL(G[1], G[1], current_A[0], batches[m][j]);	// Corresponding with T_{+}
					};
				};

				if ( (isZero_Point(G[0]) != 1) && (isZero_Point(G[1]) != 1) )	// Depending on randomness
				{
